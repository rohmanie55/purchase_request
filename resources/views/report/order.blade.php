...
<body>
    <header>
      ...
      <main>
        <div style="text-align: center;padding-top: 10px;"><h3>Laporan Purchase Order</h3></div>
        <table style="width: 100%" class="collapse">
            <tr class="collapse">
                <td class="collapse">No</td>
                <td class="collapse">No Order</td>
                <td class="collapse">Tgl Order</td>
                <td class="collapse">Nama Supplier</td>
                <td class="collapse">Telpon Supplier</td>
                <td class="collapse">Nama Barang</td>
                <td class="collapse">Harga Barang</td>
                <td class="collapse">Qty Order</td>
                <td class="collapse">Total</td>
                <td class="collapse">Approval</td>
            </tr>
            @php
                $no = 1;
            @endphp
            @foreach ($data as $order)
                ...
                <tr class="collapse">
                    <td class="collapse">{{ $no }}</td>
                    <td class="collapse">{{ $order->no_order }}</td>
                    <td class="collapse">{{ $order->tgl_order }}</td>
                    <td class="collapse">{{ $order->supplier->nm_supp }}</td>
                    <td class="collapse">{{ $order->supplier->telpon }}</td>
                    <td class="collapse">{{ $detail->barang->kd_barang }} - {{ $detail->barang->nm_brg }}</td>
                    <td class="collapse">@currency($detail->barang->harga)</td>
                    <td class="collapse">{{ $detail->qty_order }} {{ $detail->barang->unit }}</td>
                    <td class="collapse">@currency($order->total)</td>
                    <td class="collapse">{{ $order->approve->name ?? '' }}</td>
                </tr>
                @php
                    $no++;
                @endphp
